...
import {Image, Usuario} from '../models';
...
export class FileDownloadController {
  ...
  private GetFolderPathByType(type: number) {
    ...
    switch (type) {
      ...
      // product
      case 2:
        filePath = path.join(__dirname, UploadFilesKeys.MANGA_IMAGE_PATH);
        break;
      // advertising
      case 3:
  ...
  private async GetFilenameById(type: number, recordId: string) {
    ...
    switch (type) {
      // customer
      case 1:
        const usuario: Usuario = await this.customerRepository.findById(recordId);
